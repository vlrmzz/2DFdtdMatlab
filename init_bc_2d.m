% initializating PML boundary condition fot TMz
disp('Initializating PML boundary condition for TMz')
...
n_pml_xn = boundary.n_pml_xn;
n_pml_xp = boundary.n_pml_xp;
n_pml_yn = boundary.n_pml_yn;
n_pml_yp = boundary.n_pml_yp;
...
if is_pml_xn  
    ...
    sigma_max = -(pml_order+1)*eps_0*c_0*log(R_0)/(2*dx*n_pml_xn);  
...
if is_pml_xp  
    ...
    sigma_max = -(pml_order+1)*eps_0*c_0*log(R_0)/(2*dx*n_pml_xp);  
...
if is_pml_yn  
    ...
    sigma_max = -(pml_order+1)*eps_0*c_0*log(R_0)/(2*dy*n_pml_yn);  
...
if is_pml_yp  
    ...
    sigma_max = -(pml_order+1)*eps_0*c_0*log(R_0)/(2*dy*n_pml_yp);  
